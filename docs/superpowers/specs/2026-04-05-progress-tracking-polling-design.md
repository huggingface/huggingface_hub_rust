...
**Methods not covered** (can be added later):

- `download_file_stream` — returns `(Option<u64>, Box<dyn Stream<...>>)`. The consumer controls chunk consumption, so wrapping in `TransferTask` doesn't fit naturally. Progress could be added via a `ProgressHandle` field on the returned stream wrapper.
- `download_file_to_bytes` — thin wrapper around `download_file_stream`. Would inherit progress if `download_file_stream` gained it.
...
The actual logic moves to private `*_inner` methods that accept a `&ProgressHandle`. Params structs gain `Clone` derives where not already present. Currently no params structs derive `Clone`, but all their fields (`String`, `Vec`, `PathBuf`, `Option<T>`, `bool`, `AddSource`) already implement `Clone`, so adding `#[derive(Clone)]` is straightforward.
...
The blocking API (`HFClientSync`, `HFRepositorySync`) is generated by `sync_api!` macros in `macros.rs`. These macros currently assume async functions: they call `self.runtime.block_on(self.inner.method(params))`. Since the transfer methods change from `async fn -> Result<T>` to `fn -> TransferTask<T>`, the macros need a new variant.

### New `sync_api_transfer!` macro

Add to `macros.rs`:

```rust
macro_rules! sync_api_transfer {
    (
        $(#[$impl_meta:meta])*
        ...
        impl $crate::blocking::$sync_type {
            $(
                #[doc = concat!("Blocking wrapper around [`", stringify!($async_type), "::", stringify!($name), "`]. Returns a `TransferTask` that can be polled for progress or blocked on.")]
                pub fn $name(&self $(, $pname : $ptype)*) -> TransferTask<$ret> {
                    self.inner.$name($($pname),*)
                }
            )*
        }
    };
...
This variant does **not** call `block_on` — it returns the `TransferTask` directly so the caller can choose to poll progress or block.

### TransferTask::block_on

`TransferTask` gains a method that accepts an external runtime:

```rust
impl<T> TransferTask<T> {
    /// Block until the transfer completes using the provided runtime.
    /// For use with the blocking API — pass the runtime from `HFRepositorySync`.
    pub fn block_on(self, runtime: &tokio::runtime::Runtime) -> Result<T> {
        runtime.block_on(self.future)
    }
}
```

### Usage pattern in blocking callers

```rust
let sync_repo = client.model("openai-community", "gpt2");
let task = sync_repo.download_file(&params);

// Option A: just block (equivalent to the old async .await)
let path = task.block_on(&sync_repo.runtime)?;

// Option B: poll from another thread, then block
let progress = task.progress().clone();
let handle = std::thread::spawn(move || {
    loop {
        let p = progress.progress();
        println!("{}/{}", p.bytes_completed, p.total_bytes);
        if p.phase == TransferPhase::Complete { break; }
...
### sync_api! usage update

In `api/files.rs`, the transfer methods move from `sync_api!` to `sync_api_transfer!`:

```rust
// Transfer methods — return TransferTask, caller chooses to poll or block
sync_api_transfer! {
    impl HFRepository -> HFRepositorySync {
        fn download_file(&self, params: &RepoDownloadFileParams) -> PathBuf;
        fn snapshot_download(&self, params: &RepoSnapshotDownloadParams) -> PathBuf;
        fn create_commit(&self, params: &RepoCreateCommitParams) -> CommitInfo;
        fn upload_file(&self, params: &RepoUploadFileParams) -> CommitInfo;
        fn upload_folder(&self, params: &RepoUploadFolderParams) -> CommitInfo;
    }
}

// Non-transfer methods stay on sync_api! (unchanged)
sync_api! {
    impl HFRepository -> HFRepositorySync {
...
| File | Changes |
|------|---------|
| `huggingface_hub/src/types/mod.rs` | Add `pub mod progress` and re-exports. `lib.rs` already does `pub use types::*` so progress types are automatically publicly re-exported — no changes needed in `lib.rs`. |
| `huggingface_hub/src/macros.rs` | Add `sync_api_transfer!` macro variant for methods returning `TransferTask<T>` (see Blocking API section) |
| `huggingface_hub/src/api/files.rs` | Change download/upload methods from `async fn` to `fn -> TransferTask`, move logic to `*_inner` methods, add HTTP stream progress reporting. Split `sync_api!` block: transfer methods move to `sync_api_transfer!`, non-transfer methods stay on `sync_api!`. |
| `huggingface_hub/src/api/commits.rs` | `create_commit` is defined in `api/files.rs` (not `commits.rs`) — no changes here. `commits.rs` contains commit listing, diffs, branch/tag management which are not transfer operations. |
